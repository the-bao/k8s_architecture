  ...
  challenges: [
    /* ===== 挑战 1：同节点通信（动画型） ===== */
    {
      ...
      render(engine, animations, interactions, app) {
        ...
        engine.drawText('veth-A', 280, 85, { color: '#58a6ff', fontSize: 11, align: 'center', baseline: 'middle' });

        engine.drawRect(360, 45, 140, 80, {
          fillColor: '#d2992211', borderColor: '#d29922', radius: 8, id: 'bridge'
        });
        engine.drawText('🌉 cbr0 网桥', 430, 70, { color: '#d29922', fontSize: 12, fontWeight: '600', align: 'center' });
        engine.drawText('MAC 学习\nARP 解析', 430, 95, { color: '#8b949e', fontSize: 10, align: 'center' });

        engine.drawRect(530, 65, 100, 40, {
          fillColor: '#58a6ff11', borderColor: '#58a6ff', radius: 6, id: 'veth-b'
        });
        engine.drawText('veth-B', 580, 85, { color: '#58a6ff', fontSize: 11, align: 'center', baseline: 'middle' });
        ...
        engine.drawText('💡 通信原理：', 100, 195, { color: '#58a6ff', fontSize: 12, fontWeight: '600' });
        engine.drawText('1. Pod-A 通过 eth0 发送数据包（目的 IP: 10.244.1.8）', 100, 215, { color: '#8b949e', fontSize: 11 });
        engine.drawText('2. 数据包通过 veth-A 到达网桥 cbr0', 100, 232, { color: '#8b949e', fontSize: 11 });
        engine.drawText('3. 网桥查找 MAC 地址表，通过 ARP 解析将数据转发到 veth-B', 100, 249, { color: '#8b949e', fontSize: 11 });

        const btn = document.createElement('div');
        btn.style.cssText = 'position:absolute;bottom:60px;left:50%;transform:translateX(-50%);background:#238636;color:#fff;padding:8px 16px;border-radius:6px;font-size:13px;cursor:pointer;font-weight:600;z-index:100;';
    ...
    /* ===== 挑战 2：跨节点通信（动画型） ===== */
    {
      ...
      render(engine, animations, interactions, app) {
        ...
        engine.drawText('cbr0', 290, 120, { color: '#d29922', fontSize: 11, align: 'center', baseline: 'middle' });
        ...
        engine.drawText('cbr0', 690, 120, { color: '#d29922', fontSize: 11, align: 'center', baseline: 'middle' });
        ...
        const btn = document.createElement('div');
        btn.style.cssText = 'position:absolute;bottom:40px;left:50%;transform:translateX(-50%);background:#238636;color:#fff;padding:8px 16px;border-radius:6px;font-size:13px;cursor:pointer;font-weight:600;z-index:100;';
    ...
    /* ===== 挑战 4：Ingress 路由（配置型） ===== */
    {
      ...
      render(engine, animations, interactions, app) {
        ...
        engine.drawText('路由规则：', 200, 185, { color: '#e6edf3', fontSize: 12, fontWeight: '600' });
    ...
    /* ===== 挑战 5：网络策略（配置型） ===== */
    {
      ...
      render(engine, animations, interactions, app) {
        ...
        engine.drawText('🎯 安全策略要求：', 80, 195, { color: '#f85149', fontSize: 12, fontWeight: '600' });
        engine.drawText('1. 只允许 Frontend Pod 访问 Backend Pod（禁止 Internet 直接访问 Backend）', 80, 215, { color: '#8b949e', fontSize: 11 });
        engine.drawText('2. 只允许 Backend Pod 访问 Database（禁止 Frontend 直接访问 Database）', 80, 235, { color: '#8b949e', fontSize: 11 });
        engine.drawText('3. Internet 只能访问 Frontend', 80, 255, { color: '#8b949e', fontSize: 11 });
