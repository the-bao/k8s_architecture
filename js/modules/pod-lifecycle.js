  ...
  challenges: [
    /* ===== 挑战 1：发起请求（序列型） ===== */
    {
      ...
      render(engine, animations, interactions, app) {
        ...
        const steps = [
          { id: 'user-cmd', label: '用户执行 kubectl run', x: 50, y: 160, w: 190, h: 50, color: '#87867f',
            hint: '用户在终端输入 kubectl run nginx --image=nginx' },
          { id: 'read-yaml', label: '生成 Pod YAML', x: 60, y: 280, w: 170, h: 50, color: '#a08bb5',
            hint: 'kubectl 将命令行参数转换为 Pod 定义 YAML' },
          { id: 'http-post', label: '发送 HTTP POST', x: 310, y: 220, w: 160, h: 50, color: '#d97757',
            hint: 'kubectl 通过 HTTP POST 将 YAML 发送到 API Server 的 /api/v1/namespaces/default/pods' },
          { id: 'api-receive', label: 'API Server 接收', x: 540, y: 160, w: 150, h: 50, color: '#d97757',
        ...
        engine.drawArrow(145, 210, 145, 280, { color: '#5e5d59', dashed: [3, 3] });
        engine.drawArrow(230, 305, 310, 245, { color: '#5e5d59', dashed: [3, 3] });
        engine.drawArrow(470, 245, 540, 185, { color: '#5e5d59', dashed: [3, 3] });
        ...
        interactions.enableSequence(correctOrder, {
          onCorrectStep(id, idx) {
            ...
            app.showFeedback('success', '✅ ' + step.hint, 1500);
          },
    ...
    /* ===== 挑战 2：认证鉴权（选择题） ===== */
    {
      ...
      render(engine, animations, interactions, app) {
        ...
        engine.drawNode(280, 60, 200, 60, 'API Server', {
          borderColor: '#d97757', bgColor: '#d9775722'
        });

        engine.drawArrow(150, 90, 280, 90, { color: '#7c9a5e' });
        engine.drawText('Pod 创建请求', 80, 75, { color: '#7c9a5e', fontSize: 12 });

        engine.drawRect(200, 180, 360, 80, {
          fillColor: '#c49a3c11', borderColor: '#c49a3c', radius: 10
        });
        engine.drawText('🔐 认证关卡', 380, 195, {
          color: '#c49a3c', fontSize: 13, fontWeight: '600', align: 'center'
        });
        ...
        engine.drawRect(200, 310, 360, 80, {
          fillColor: '#c9644211', borderColor: '#c96442', radius: 10
        });
        ...
        engine.drawArrow(380, 260, 380, 310, { color: '#5e5d59', dashed: [3, 3] });
        ...
        interactions.enableChoice(
          ...
          (correct, id) => {
            if (correct) {
              authPassed = true;
              app.showFeedback('success', '✅ 正确！ServiceAccount 使用 JWT Token 认证。API Server 验证 Token 签名后确认身份。', 1500);
              setTimeout(() => {
                interactions.enableChoice(
                  [
                  ...
                  (correct2) => {
                    if (correct2) {
                      app.showFeedback('success', '✅ 正确！K8s 默认使用 RBAC。RoleBinding 将 ServiceAccount 绑定到具有 Pod 创建权限的 Role。', 1500);
                      setTimeout(() => app.onChallengeComplete('pod-lifecycle', 1, false), 1500);
                    } else {
                      app.showFeedback('error', '❌ K8s 默认的鉴权方式是 RBAC（基于角色的访问控制）。', 1500);
                    }
                  }
                );
              }, 1000);
            } else {
              app.showFeedback('error', '❌ ServiceAccount 不使用用户名密码认证。它使用自动挂载的 JWT Token（在 /var/run/secrets/kubernetes.io/serviceaccount/ 下）。', 1500);
            }
          }
        );
      }
    ...
    /* ===== 挑战 3：持久化（序列型） ===== */
    {
      ...
      render(engine, animations, interactions, app) {
        ...
        const steps = [
          { id: 'validate', label: '验证 YAML 格式', x: 80, y: 100, w: 150, h: 50, color: '#c49a3c',
            hint: 'API Server 校验 YAML 的 schema 是否合法' },
          { id: 'admission', label: '准入控制器', x: 80, y: 220, w: 150, h: 50, color: '#c96442',
            hint: 'MutatingWebhook 修改默认值 → ValidatingWebhook 校验策略' },
          { id: 'encode', label: '编码为 JSON', x: 310, y: 160, w: 140, h: 50, color: '#d97757',
            hint: '内部将 YAML 编码为 JSON protobuf 格式' },
          { id: 'write-etcd', label: '写入 etcd', x: 530, y: 100, w: 130, h: 50, color: '#a08bb5',
            hint: 'Pod 定义持久化到 etcd，状态为 Pending' },
          { id: 'confirm', label: '返回确认', x: 530, y: 220, w: 130, h: 50, color: '#7c9a5e',
        ...
        engine.drawText('etcd', 600, 80, { color: '#a08bb5', fontSize: 14, fontWeight: '600', align: 'center' });
        ...
        interactions.enableSequence(correctOrder, {
          ...
          onWrongStep(id, expectedIdx) {
            app.showFeedback('error', '❌ 顺序不对！下一步：' + steps[expectedIdx].hint, 1500);
          },
          onComplete() {
    ...
    /* ===== 挑战 4：调度决策（决策型） ===== */
    {
      ...
      render(engine, animations, interactions, app) {
        ...
        engine.drawNode(280, 30, 180, 50, 'kube-scheduler', {
          borderColor: '#7c9a5e', bgColor: '#7c9a5e22'
        });
        ...
        engine.drawNode(60, 40, 140, 40, 'Nginx Pod', {
          borderColor: '#a08bb5', bgColor: '#a08bb511'
        });
        engine.drawArrow(200, 60, 280, 55, { color: '#c49a3c' });

        const nodes = [
          { id: 'node1', label: 'Node-1', x: 80, y: 160, w: 180, h: 120,
            color: '#7c9a5e', cpu: '40%', mem: '35%', status: '资源充足', schedulable: true },
          { id: 'node2', label: 'Node-2', x: 310, y: 160, w: 180, h: 120,
            color: '#b53333', cpu: '92%', mem: '88%', status: '资源不足', schedulable: false },
          { id: 'node3', label: 'Node-3', x: 540, y: 160, w: 180, h: 120,
            color: '#c49a3c', cpu: '75%', mem: '70%', status: '资源紧张', schedulable: false }
        ];
        ...
        interactions.onClick(nodeIds, (id, data) => {
          if (data.schedulable) {
            animations.emitParticles(engine, data.x + data.w / 2, data.y + data.h / 2, '#7c9a5e', 20);
            ...
            app.showFeedback('error', '❌ ' + reason + '提示：选择资源最充足的节点。', 1500);
          }
        });
      }
    },
    ...
    /* ===== 挑战 5：启动容器（序列型） ===== */
    {
      ...
      render(engine, animations, interactions, app) {
        ...
        const steps = [
          { id: 'watch', label: 'kubelet Watch', x: 60, y: 100, w: 140, h: 50, color: '#b53333',
            hint: 'kubelet 通过 Watch 机制监听 API Server，发现新 Pod 被调度到本节点' },
          { id: 'validate', label: '校验 Pod 规格', x: 60, y: 220, w: 140, h: 50, color: '#c49a3c',
            hint: 'kubelet 校验 Pod 定义（镜像、资源、挂载卷等）是否合法' },
          { id: 'pull', label: '拉取镜像', x: 260, y: 160, w: 130, h: 50, color: '#d97757',
            hint: '容器运行时（containerd）从镜像仓库拉取 nginx:latest' },
          { id: 'mount', label: '挂载存储卷', x: 440, y: 100, w: 130, h: 50, color: '#a08bb5',
            hint: '如果 Pod 定义了 Volume，先挂载存储卷到指定路径' },
          { id: 'create', label: '创建容器', x: 440, y: 220, w: 130, h: 50, color: '#7c9a5e',
            hint: '容器运行时根据镜像创建容器进程，配置网络命名空间' },
          { id: 'probe', label: '健康检查', x: 620, y: 160, w: 120, h: 50, color: '#c96442',
        ...
        engine.drawRect(230, 60, 370, 240, {
          fillColor: '#141413', borderColor: '#7c9a5e', radius: 12
        });
